  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    ci = cinfo->MCU_membership[blkn];
		...
		*/
		...
		/*for noDC-mode, if consider the grey values, only increment the dc-counter*/
		if(cinfo->noDC && ci == 0){
			...
			/*for DC_to_zero-mode, set the DC's of the grey-values to zero */
			if(cinfo->DC_to_zero){
				
				temp = ci == 0 ? 0 : temp2 - entropy->saved.last_dc_val[ci];
